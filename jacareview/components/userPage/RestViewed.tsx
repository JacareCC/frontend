...
const RestViewed: React.FC<RestViewedProps> = ({ historyData }) => {
  ...
  function filterToUniqueRestaurants(myArr: any, key: string) {
    if (Array.isArray(myArr)) {
      let arrayWithSaves = myArr.filter(
        (element: any) => element.saved === true
      );
      let newArrayWithSaves = myArr.filter((element: any) => {
        return (
          arrayWithSaves.find(
            (uniqueRestaurant) =>
              uniqueRestaurant.restaurant_id_id == element.restaurant_id_id
          ) == undefined
        );
      });

      let newArrayToFilter = arrayWithSaves.concat(newArrayWithSaves);
      let testFilteredHistory = newArrayToFilter.filter(
      ...
      setHistoryDataFiltered([]);
    }
  }

  return (
    <div className="flex flex-col align-center items-center bg-test pb-4">
      <h1 className="pt-2 text-l font-semibold text-xl my-2">
        Viewed Restaurants
      </h1>
              ...
              <div className="card w-full max-h-[400px] md:max-h-[600px] overflow-y-scroll scrollbar-thin px-4 py-2">
                {historyDataFiltered &&
                  historyDataFiltered.map((element: any, index: number) => (
                    <div
                      key={`z${index}`}
                      className="flex flex-col border-b mb-2 p-4 bg-test"
                    >
                      <div key={`b${index}`} className="">
                        <div
                          key={`c${index}`}
                          className="mb-1 border-b text-center pb-2"
                        >
                        ...
                        <div className="flex flex-col gap-4">
                          <Link
                            ...
                            className="bg-jgreen  text-jyellow p-2 rounded shadow-lg shadow-xl flex justify-center items-center gap-4"
                          >
