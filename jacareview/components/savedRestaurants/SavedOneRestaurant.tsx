...
const SavedOneRestaurant: React.FC<SavedOneProps> = ({location, savedData, setRandomOneClicked}) => {
    ...
    function goBack(){
        ...
        setRandomOneClicked((prev:boolean) => {!prev})
    }
    ...
    return (
        ...
        { randomRestaurant &&(<div className="bg-test flex flex-col items-center shadow-xl w-11/12 mx-6 my-2 rounded bg-gray-100 text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl px-2 py-4">
          <div className="flex items-center justify-center">
        <button onClick={handleReroll} className="bg-gradient-to-r mt-4 ml-4 from-yellow-500 via-red-500 to-pink-500 text-white px-6 py-3 rounded shadow-md hover:scale-105 transform transition-transform duration-300">
                    Surprise me Again!
                    </button>
                    </div>
                  ...
                  (<div key="currentDistance" className="md:pl-2">
                    Current Distance: {getDistanceInApproxKm(randomRestaurant.location, location)}
                    </div>)}
                  <Link  href={`/review/?restaurant=${randomRestaurant.restaurant_id_id}`} className="w-full mt-2 bg-jgreen text-jyellow p-2 rounded shadow-lg shadow-xl flex justify-center items-center gap-4">
                    Review
                    <Star />
                  </Link>
                    <div className="flex w-full items-center">
                      <a className='w-full flex items-center justify-center rounded bg-secl text-white px-4 py-2 mt-2' href={`https://www.google.com/maps/place/?q=place_id:${randomRestaurant.googlePlaceId} `} target='_blank'>
                        Go to map
                      </a>
                    </div>
                    <div className="flex flex-row">
                    <button onClick={goBack} className="bg-jgreen text-white px-4 py-2 rounded mt-4">
                          Back to Full List
                        </button>
