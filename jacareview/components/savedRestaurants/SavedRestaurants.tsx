...
const SavedRestaurants: React.FC<SavedOneRestaurantsProps> = ({
  setRandomOneClicked,
  randomOneClicked,
}) => {
  ...
  useEffect(() => {
    if (fetchMadeForFilter) {
      if (Array.isArray(savedData)) {
        let filteredHistory = savedData.filter(
          (obj: any, pos: any, arr: any) => {
            return (
              arr
                .map((mapObj: any) => mapObj["date_visited"])
                .indexOf(obj["date_visited"]) === pos
            );
          }
        );
  ...
  async function getSavedRestaurants() {
    try {
      const results = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}user/favorites/all`,
        {
          method: "GET",
          headers: {
            ...
            <div className="flex flex-col align-center items-center w-full">
              ...
              {fetchedData &&
                ...
                [...savedData].map((element: any, index: number) => (
                  <div
                    className="bg-test flex flex-col items-start shadow-xl w-11/12 mx-6 my-2 rounded bg-gray-100 text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl px-2 py-4"
                    key={`a${index}`}
                  >
                    ...
                    <Link
                      key={`d${index}`}
                      href={`/review/?restaurant=${element.restaurant_id_id}`}
                    ...
                    <div className="flex w-full items-center gap-2">
                      ...
                      <div className="flex w-full items-center">
                        <a
                          className="w-full flex items-center justify-center gap-2 rounded bg-secl text-white px-4 py-2 mt-2"
                          href={`https://www.google.com/maps/search/?api=1&query=${element.name.replace(
                            / /g,
                            "+"
                          )}&location=${element.location.latitude},${
                            element.location.longitude
                          }&query._place_id=${element.googlePlaceId}`}
                          target="_blank"
                        >
                          Go to maps
              ...
              {fetchedData && randomOneClicked && (
                <SavedOneRestaurant
                  location={location}
                  savedData={savedData}
                  setRandomOneClicked={setRandomOneClicked}
                />
              )}
            </div>
          </div>
